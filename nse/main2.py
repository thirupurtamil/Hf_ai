...
def process_option_chain(opt_data, quote_data, underlying_value):
    ...
    for item in quote_data.get('stocks', []):
        ...
        quote_dict[key] = {
            ...
            # Assuming yesterday_high and yesterday_low are not directly available; using current high/low as placeholders
    ...
    # price_strategy தனி அட்டவணைக்கான தரவு
    price_strategy_data = []
    ...
    for _, row in rawop.iterrows():
        ...
        # price_strategy கணக்கிடுதல்
        call_price_symbol, call_price_diff = price_strategy(
            call_quote['open'],
            call_quote['prev_close'],
            call_quote['yesterday_high'],
            call_quote['yesterday_low']
        )
        put_price_symbol, put_price_diff = price_strategy(
            put_quote['open'],
            put_quote['prev_close'],
            put_quote['yesterday_high'],
            put_quote['yesterday_low']
        )
        
        # price_strategy தரவை தனி அட்டவணைக்கு சேர்த்தல்
        if call_quote['open'] != 0:  # கால் தரவு இருந்தால் மட்டும் சேர்க்கவும்
        ...
        if put_quote['open'] != 0:  # புட் தரவு இருந்தால் மட்டும் சேர்க்கவும்
            price_strategy_data.append({
                'Strike Price': strike,
                'Option Type': 'Put',
        ...
        full_data.append({
            ...
            'Call Price Symbol': call_price_symbol,  # New column
            'Call Price Diff': call_price_diff,     # New column
            'Strike Price': strike,
            ...
            'Put Price Symbol': put_price_symbol,   # New column
            'Put Price Diff': put_price_diff        # New column
        })

    full_df = pd.DataFrame(full_data)
    price_strategy_df = pd.DataFrame(price_strategy_data)
    ...
    for _, row in filtered_df.iterrows():
        for t in ['Call', 'Put']:
            if row[f'{t} Volume'] != 0:
                ...
                # price_strategy உயர் வால்யூம் தரவுக்கு
                price_symbol, price_diff = price_strategy(
                    row[f'{t} Open'],
                    quote['prev_close'],
                    quote['yesterday_high'],
                    quote['yesterday_low']
                )
                ...
                high_vol_data.append({
                    ...
                    'Strategy': strategy,
                    'Price Symbol': price_symbol,  # New column
                    'Price Diff': price_diff       # New column
                })
...
def main():
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        print(high_vol_df.to_string(index=False))

        print("\n=== Price Strategy Table ===")
        print(price_strategy_df.to_string(index=False))
