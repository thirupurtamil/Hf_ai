...
def process_option_chain(opt_data, quote_data, underlying_value):
    ...
    for item in quote_data.get('stocks', []):
        ...
        quote_dict[key] = {
            ...
            'yesterday_high': item['metadata'].get('prevHigh', item['metadata']['highPrice']),
            'yesterday_low': item['metadata'].get('prevLow', item['metadata']['lowPrice'])
        }
    ...
    for _, row in rawop.iterrows():
        ...
        expiry = opt_data['records']['expiryDates'][0] if 'records' in opt_data and opt_data['records'].get('expiryDates') else ''
        
        call_key = (strike, 'Call', expiry)
        put_key = (strike, 'Put', expiry)
        call_quote = quote_dict.get(call_key, {'open': 0, 'high': 0, 'low': 0, 'close': 0, 'prev_close': 0, 'value': 0, 'yesterday_high': 0, 'yesterday_low': 0})
        put_quote = quote_dict.get(put_key, {'open': 0, 'high': 0, 'low': 0, 'close': 0, 'prev_close': 0, 'value': 0, 'yesterday_high': 0, 'yesterday_low': 0})
        
        call_price_change = call_quote['close'] - call_quote['prev_close'] if call_quote['prev_close'] != 0 else call_quote['close'] - call_quote['open']
        put_price_change = put_quote['close'] - put_quote['prev_close'] if put_quote['prev_close'] != 0 else put_quote['close'] - put_quote['open']
        ...
        call_price_symbol, call_price_diff = price_strategy(
            call_quote['open'], call_quote['prev_close'],
            call_quote['yesterday_high'], call_quote['yesterday_low']
        )
        put_price_symbol, put_price_diff = price_strategy(
            put_quote['open'], put_quote['prev_close'],
            put_quote['yesterday_high'], put_quote['yesterday_low']
        )
        
        if call_quote['open'] != 0:
            price_strategy_data.append({
                'Strike Price': strike,
                'Option Type': 'Call',
                'Open': round(call_quote['open'], 2),
                'High': round(call_quote['high'], 2),
                'Low': round(call_quote['low'], 2),
                'Close': round(call_quote['close'], 2),
                'Price Strategy': call_price_symbol,
                'Price Diff': call_price_diff
            })
        if put_quote['open'] != 0:
            price_strategy_data.append({
                'Strike Price': strike,
                'Option Type': 'Put',
                'Open': round(put_quote['open'], 2),
                'High': round(put_quote['high'], 2),
                'Low': round(put_quote['low'], 2),
                'Close': round(put_quote['close'], 2),
                'Price Strategy': put_price_symbol,
                'Price Diff': put_price_diff
            })
        ...
        full_data.append({
            ...
            'Call Price Symbol': call_price_symbol,
            'Call Price Diff': call_price_diff,
            'Strike Price': strike,
            ...
            'Put Price Symbol': put_price_symbol,
            'Put Price Diff': put_price_diff
        })

    full_df = pd.DataFrame(full_data) if full_data else pd.DataFrame()
    price_strategy_df = pd.DataFrame(price_strategy_data) if price_strategy_data else pd.DataFrame()
    ...
    if not filtered_df.empty and len(filtered_df) != 31:
        print(f"எச்சரிக்கை: 31 வரிசைகள் எதிர்பார்க்கப்பட்டன, ஆனால் {len(filtered_df)} வரிசைகள் கிடைந்தன.")

    high_vol_data = []
    if not filtered_df.empty:
        for _, row in filtered_df.iterrows():
            for t in ['Call', 'Put']:
                if row.get(f'{t} Volume', 0) != 0:
                    expiry_for_quote = opt_data['records']['expiryDates'][0] if opt_data and 'records' in opt_data and opt_data['records'].get('expiryDates') else ''
                    quote_key = (row['Strike Price'], t, expiry_for_quote)
                    quote = quote_dict.get(quote_key, {'close': 0, 'prev_close': 0, 'open': 0, 'yesterday_high': 0, 'yesterday_low': 0})
                    ...
                    price_symbol, price_diff = price_strategy(
                        row[f'{t} Open'], quote['prev_close'],
                        quote['yesterday_high'], quote['yesterday_low']
                    )
    ...
    high_vol_df = pd.DataFrame(high_vol_data).sort_values(by='Volume', ascending=False).head(20).reset_index(drop=True) if high_vol_data else pd.DataFrame()
    if not high_vol_df.empty:
        high_vol_df['S.No'] = high_vol_df.index + 1
        high_vol_df = high_vol_df[['S.No', 'Strike Price', 'Type', 'Volume', 'OI', 'Change OI', 'LTP', 'IV', 'Open', 'High', 'Low', 'Close', 'Value', 'Strategy', 'Price Symbol', 'Price Diff']]

    return filtered_df, high_vol_df, price_strategy_df
...
def check_ltp_similarity(filtered_df, opt_data):
    ...
    matches = []
    for _, row in filtered_df.iterrows():
        call_ltp = row.get('Call LTP', 0)
        put_ltp = row.get('Put LTP', 0)
    ...
    if not matches:
        ...
        valid_rows = filtered_df[(filtered_df.get('Call LTP', 0) != 0) & (filtered_df.get('Put LTP', 0) != 0)].copy()
...
def analyze_max_oi_value_pair(filtered_df):
    ...
    filtered_df['Total OI'] = filtered_df.get('Call OI', 0) + filtered_df.get('Put OI', 0)
    max_oi_row = filtered_df.loc[filtered_df['Total OI'].idxmax()]
    
    strike = int(max_oi_row['Strike Price'])
    call_close = round(max_oi_row.get('Call Close', 0), 2)
    put_close = round(max_oi_row.get('Put Close', 0), 2)
    call_value = round(max_oi_row.get('Call Value', 0), 2)
    put_value = round(max_oi_row.get('Put Value', 0), 2)
    ...
    result = {
        ...
        'BEP1 (கால் க்ளோஸ் + ஸ்ட்ரைப்)': bep1,
        'BEP2 (புட் க்ளோஸ் - ஸ்ட்ரைப்)': bep2,
...
def analyze_max_value_pair(filtered_df):
    ...
    filtered_df['Total Value'] = filtered_df.get('Call Value', 0) + filtered_df.get('Put Value', 0)
    max_value_row = filtered_df.loc[filtered_df['Total Value'].idxmax()]
    
    strike = int(max_value_row['Strike Price'])
    call_close = round(max_value_row.get('Call Close', 0), 2)
    put_close = round(max_value_row.get('Put Close', 0), 2)
    call_value = round(max_value_row.get('Call Value', 0), 2)
    put_value = round(max_value_row.get('Put Value', 0), 2)
...
def main():
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        filtered_df, high_vol_df, price_strategy_df = process_option_chain(opt_data, quote_data, underlying_value)
        ...
        summary_data = importdata(opt_data)

        print("\n=== Option Chain Summary ===")
        print(summary_data)

        print("\n=== LTP Similarity Analysis ===")
        print(ltp_similarity)

        print("\n=== Max OI Pair Analysis ===")
        print(max_oi_pair)

        print("\n=== Max Value Pair Analysis ===")
        print(max_value_pair)

        print("\n=== High Volume Analysis ===")
        print(high_volume_analysis)

        print("\n=== Filtered Option Chain (Top 31 Rows) ===")
        print(filtered_df.to_string(index=False) if not filtered_df.empty else "No filtered data")

        print("\n=== High Volume Data (Top 20) ===")
        print(high_vol_df.to_string(index=False) if not high_vol_df.empty else "No high volume data")

        # Price Strategy Table → Top 18 by max Volume (fallback to head(18) if no volume info)
        if not price_strategy_df.empty:
            if not high_vol_df.empty:
                price_strategy_df['Volume'] = price_strategy_df['Strike Price'].map(
                    lambda sp: int(high_vol_df.loc[high_vol_df['Strike Price'] == sp, 'Volume'].max())
                    if not high_vol_df.loc[high_vol_df['Strike Price'] == sp].empty else 0
                )
                price_strategy_df = price_strategy_df.sort_values(by='Volume', ascending=False).head(18).drop(columns=['Volume'])
            else:
                price_strategy_df = price_strategy_df.head(18)

        print("\n=== Price Strategy Table (Top 18 by Volume) ===")
        print(price_strategy_df.to_string(index=False) if not price_strategy_df.empty else "No price strategy data")
